...
Subset variables are renamed for clarity and to align with the concepts as operationalized.
...
ggplot(data = wav49_factored, aes(x = GovtReg, fill = factor(GovtReg))) +
    geom_bar(position = "dodge") +
 scale_fill_manual(values = c("#BCB88A",
                                        "#FFFDD0"))
ggsave("base.png")
```
...
ggplot(data = wav49_factored, aes(x = GovtReg, fill = Party)) +
    geom_bar(position = "dodge") +
  scale_fill_manual(values = c("#660000",
                                        "#BCB88A"))
ggsave("party.png")
```
